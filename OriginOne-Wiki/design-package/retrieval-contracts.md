...
| 文章作者 | 写教程时需要截图 | task-end | terminal screenshot PNG | 插入文章说明流程 | medium | local public-safe |
...
检索路径必须先看 `wiki/index.md` 和长期 wiki 页面；如果 wiki 不够，再回 raw 查证据。这个 demo 暂时用简单关键词评分模拟检索，文章要明确说明：脚本不是模型，它只是让新手先看见流程。
