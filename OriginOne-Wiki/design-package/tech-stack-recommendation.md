...
- sharing_requirement：文章、截图、目录包。
- audit_requirement：轻量，可用 output 和 transcript 说明。
- maintenance_capacity：低。

## recommendation

使用 Markdown + Python 脚本 + 终端截图。
...
- Web App：展示效果更好，但会增加前端、服务、端口和截图复杂度。
...
本地运行几乎无成本。唯一要求是 Python3 和 Pillow；Pillow 只用于生成截图，不影响核心 demo。
